...
def letters_range(*args, **kwargs):
    list_of_letters = []
    letters = list(i for i in range(97, 123))

    if len(args) == 1:
        for i in range(letters[0], ord(args[0])):
            list_of_letters.append(chr(i))
    elif len(args) == 2:
        for i in range(ord(args[0]), ord(args[1])):
            list_of_letters.append(chr(i))
    elif len(args) == 3:
        for i in range(ord(args[0]), ord(args[1]), int(args[2])):
            list_of_letters.append(chr(i))
    for key in kwargs:
        list_of_letters[list_of_letters.index(key)] = str(kwargs[key])
